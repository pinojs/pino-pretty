...
for (const ignoreKeys of [
  ...
  describe(`#filterLog with an includeKeys option when the ignoreKeys being ${ignoreKeys}`, () => {
    ...
    test('filterLog include multiple nested entries under same parent', t => {
      const log = {
        level: 30,
        time: 1522431328992,
        user: { id: '1234', name: 'Joe' },
      ...
      const result = filterLog({
        ...
        context: {
          ...context,
          ignoreKeys,
          includeKeys: ['user.id', 'user.name']
        }
      })
      t.assert.deepStrictEqual(result, { user: { id: '1234', name: 'Joe' } })
    })
    ...
    test('filterLog include nested and top-level entries', t => {
      ...
      const result = filterLog({
        ...
        context: {
          ...context,
          ignoreKeys,
          includeKeys: ['time', 'user.id']
        }
      })
      t.assert.deepStrictEqual(result, { time: 1522431328992, user: { id: '1234' } })
    })

    test('filterLog include non-existent nested key returns empty', t => {
      const result = filterLog({
        log: logData,
        context: {
          ...context,
          ignoreKeys,
          includeKeys: ['data1.nonexistent']
        }
      })
      t.assert.deepStrictEqual(result, {})
    })
...
describe('#filterLog include with escaped dot keys', () => {
  test('filterLog include entry with escape sequence nested', t => {
    const result = filterLog({
      log: logData2,
      context: {
        ...context,
    ...
    t.assert.deepStrictEqual(result, { 'logging.domain.corp/operation': { producer: 'bar' } })
  })
})
