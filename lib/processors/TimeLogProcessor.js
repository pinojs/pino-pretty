...
/**
 * Prettifies a timestamp if the given `input` object has either `time`, `timestamp`
 * or a custom specified timestamp property and assigns the prettified timestamp
 * string to the `context.prettified.prettifiedTime` property.
 *
 * If a timestamp property cannot be found, then nothing will happen.
 *
 * @param {object} input The log object with the timestamp to be prettified.
 * @param {object} context The context object.
 * @param {string} [context.timestampKey='time'] The log property that should be used
...
function prettifyTime (input, { prettified, timestampKey = TIMESTAMP_KEY, translateFormat = undefined } = {}) {
  if (timestampKey in input || 'timestamp' in input) {
    const timestamp = input[timestampKey] || input.timestamp
    const formatted = translateFormat ? formatTime(timestamp, translateFormat) : timestamp
