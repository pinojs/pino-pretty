 ...
 */
function prettifyLevel (input, { colorizer = defaultColorizer } = {}) {
  if ('level' in input === false) return undefined
  return colorizer(input.level)
}
 ...
 */
function prettifyMessage (input, { messageKey = MESSAGE_KEY, colorizer = defaultColorizer } = {}) {
  if (messageKey in input === false) return undefined
  if (typeof input[messageKey] !== 'string') return undefined
  return colorizer.message(input[messageKey])
}
 ...
 */
function prettifyMetadata (input) {
 ...
 * string.
 */
function prettifyTime (input, { timestampKey = TIMESTAMP_KEY, translateFormat = undefined } = {}) {
  if (timestampKey in input === false && 'timestamp' in input === false) return undefined
  if (translateFormat) {
    return '[' + formatTime(input[timestampKey] || input.timestamp, translateFormat) + ']'
  }
  return `[${input[timestampKey] || input.timestamp}]`
}
