 ...
 *
 * @returns {undefined|string} If the message key is not found
 * then `undefined` will be returned. Otherwise, a string
 * that is the prettified message or stringified value of the message key.
 */
function prettifyMessage ({ log, messageFormat, messageKey = MESSAGE_KEY, colorizer = defaultColorizer, levelLabel = LEVEL_LABEL }) {
  ...
  if (typeof log[messageKey] !== 'string') return colorizer.message(JSON.stringify(log[messageKey]))
