...
function prettifyObject ({
  ...
  const keysToIterate = safeObjectKeys()
  var i = 0
  for (i; i < keysToIterate.length; i += 1) {
  ...
  /**
   * Should return a sanitized list of ignored keys
   *
   * @returns {iterable}
   */
  function safeObjectKeys () {
    const safePrims = objectKeys.filter(k => k !== 'null')
    const strArr = keysToIgnore.join(',').split(',')
    const sanitized = safePrims.filter(k => strArr.includes(k) === false)
    return sanitized
  }
}
